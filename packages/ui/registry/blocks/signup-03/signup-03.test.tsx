/**
 * `signup-03.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Signup03Page from "./page";

test("signup-03 renders its composition", async () => {
  const screen = await render(<Signup03Page />);
  await expect
    .element(screen.getByText("Create your account").first())
    .toBeInTheDocument();
});

test("signup-03 is axe-clean", async () => {
  const screen = await render(<Signup03Page />);
  await expect
    .element(screen.getByText("Create your account").first())
    .toBeInTheDocument();
