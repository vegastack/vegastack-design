/**
 * `sidebar-08.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar08Page from "./page";

test("sidebar-08 renders its composition", async () => {
  const screen = await render(<Sidebar08Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
});

test("sidebar-08 is axe-clean", async () => {
  const screen = await render(<Sidebar08Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
