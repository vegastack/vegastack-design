/**
 * `login-02.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Login02Page from "./page";

test("login-02 renders its composition", async () => {
  const screen = await render(<Login02Page />);
  await expect
    .element(screen.getByText("Login to your account").first())
    .toBeInTheDocument();
});

test("login-02 is axe-clean", async () => {
  const screen = await render(<Login02Page />);
  await expect
    .element(screen.getByText("Login to your account").first())
    .toBeInTheDocument();
