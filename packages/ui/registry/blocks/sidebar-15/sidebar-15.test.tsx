/**
 * `sidebar-15.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar15Page from "./page";
...
test("sidebar-15 renders its composition", async () => {
  const screen = await render(<Sidebar15Page />);
  await expect
    .element(screen.getByText("Project Management & Task Tracking").first())
    .toBeInTheDocument();
});

test("sidebar-15 is axe-clean", async () => {
  const screen = await render(<Sidebar15Page />);
  await expect
    .element(screen.getByText("Project Management & Task Tracking").first())
    .toBeInTheDocument();
