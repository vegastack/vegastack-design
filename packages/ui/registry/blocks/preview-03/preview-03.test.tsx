/**
 * `preview-03.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Preview03Page from "./index";

test("preview-03 renders its composition", async () => {
  const screen = await render(<Preview03Page />);
  await expect
    .element(screen.getByText("Preview 03").first())
    .toBeInTheDocument();
});

test("preview-03 is axe-clean", async () => {
  const screen = await render(<Preview03Page />);
  await expect
    .element(screen.getByText("Preview 03").first())
    .toBeInTheDocument();
