/**
 * `sidebar-01.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
test("sidebar-01 renders its composition", async () => {
  const screen = await render(<Sidebar01Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
});

test("sidebar-01 is axe-clean", async () => {
  const screen = await render(<Sidebar01Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
