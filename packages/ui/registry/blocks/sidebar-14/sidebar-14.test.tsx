/**
 * `sidebar-14.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar14Page from "./page";
...
test("sidebar-14 renders its composition", async () => {
  const screen = await render(<Sidebar14Page />);
  await expect
    .element(screen.getByText("Table of Contents").first())
    .toBeInTheDocument();
});

test("sidebar-14 is axe-clean", async () => {
  const screen = await render(<Sidebar14Page />);
  await expect
    .element(screen.getByText("Table of Contents").first())
    .toBeInTheDocument();
