/**
 * `board-01.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
test("board-01 renders its composition", async () => {
  const screen = await render(<Board01Page />);
  await expect
    .element(screen.getByText("New task").first())
    .toBeInTheDocument();
});

test("board-01 is axe-clean", async () => {
  const screen = await render(<Board01Page />);
  await expect
    .element(screen.getByText("New task").first())
    .toBeInTheDocument();
