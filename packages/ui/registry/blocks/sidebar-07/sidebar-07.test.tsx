/**
 * `sidebar-07.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar07Page from "./page";

test("sidebar-07 renders its composition", async () => {
  const screen = await render(<Sidebar07Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
});

test("sidebar-07 is axe-clean", async () => {
  const screen = await render(<Sidebar07Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
