/**
 * `login-05.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Login05Page from "./page";

test("login-05 renders its composition", async () => {
  const screen = await render(<Login05Page />);
  await expect
    .element(screen.getByText("Welcome to Acme Inc.").first())
    .toBeInTheDocument();
});

test("login-05 is axe-clean", async () => {
  const screen = await render(<Login05Page />);
  await expect
    .element(screen.getByText("Welcome to Acme Inc.").first())
    .toBeInTheDocument();
