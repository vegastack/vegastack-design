/**
 * `sidebar-16.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar16Page from "./page";

test("sidebar-16 renders its composition", async () => {
  const screen = await render(<Sidebar16Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
});

test("sidebar-16 is axe-clean", async () => {
  const screen = await render(<Sidebar16Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
