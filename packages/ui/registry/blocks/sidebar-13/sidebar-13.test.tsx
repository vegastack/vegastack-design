/**
 * `sidebar-13.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar13Page from "./page";
...
test("sidebar-13 renders its composition", async () => {
  const screen = await render(<Sidebar13Page />);
  await expect
    .element(screen.getByText("Open Dialog").first())
    .toBeInTheDocument();
});

test("sidebar-13 is axe-clean", async () => {
  const screen = await render(<Sidebar13Page />);
  await expect
    .element(screen.getByText("Open Dialog").first())
    .toBeInTheDocument();
