/**
 * `signup-01.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Signup01Page from "./page";

test("signup-01 renders its composition", async () => {
  const screen = await render(<Signup01Page />);
  await expect
    .element(screen.getByText("Create an account").first())
    .toBeInTheDocument();
});

test("signup-01 is axe-clean", async () => {
  const screen = await render(<Signup01Page />);
  await expect
    .element(screen.getByText("Create an account").first())
    .toBeInTheDocument();
