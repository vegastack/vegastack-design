/**
 * `sidebar-09.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar09Page from "./page";
...
test("sidebar-09 renders its composition", async () => {
  const screen = await render(<Sidebar09Page />);
  await expect
    .element(screen.getByText("All Inboxes").first())
    .toBeInTheDocument();
});

test("sidebar-09 is axe-clean", async () => {
  const screen = await render(<Sidebar09Page />);
  await expect
    .element(screen.getByText("All Inboxes").first())
    .toBeInTheDocument();
