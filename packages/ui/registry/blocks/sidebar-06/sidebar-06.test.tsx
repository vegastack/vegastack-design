/**
 * `sidebar-06.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar06Page from "./page";
...
test("sidebar-06 renders its composition", async () => {
  const screen = await render(<Sidebar06Page />);
  await expect
    .element(screen.getByText("Subscribe to our newsletter").first())
    .toBeInTheDocument();
});

test("sidebar-06 is axe-clean", async () => {
  const screen = await render(<Sidebar06Page />);
  await expect
    .element(screen.getByText("Subscribe to our newsletter").first())
    .toBeInTheDocument();
