/**
 * `signup-04.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Signup04Page from "./page";

test("signup-04 renders its composition", async () => {
  const screen = await render(<Signup04Page />);
  await expect
    .element(screen.getByText("Create your account").first())
    .toBeInTheDocument();
});

test("signup-04 is axe-clean", async () => {
  const screen = await render(<Signup04Page />);
  await expect
    .element(screen.getByText("Create your account").first())
    .toBeInTheDocument();
