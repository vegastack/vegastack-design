/**
 * `sidebar-11.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar11Page from "./page";

test("sidebar-11 renders its composition", async () => {
  const screen = await render(<Sidebar11Page />);
  await expect.element(screen.getByText("Changes").first()).toBeInTheDocument();
});

test("sidebar-11 is axe-clean", async () => {
  const screen = await render(<Sidebar11Page />);
  await expect.element(screen.getByText("Changes").first()).toBeInTheDocument();
