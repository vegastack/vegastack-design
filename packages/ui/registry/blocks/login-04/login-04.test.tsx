/**
 * `login-04.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Login04Page from "./page";

test("login-04 renders its composition", async () => {
  const screen = await render(<Login04Page />);
  await expect
    .element(screen.getByText("Login to your Acme Inc account").first())
    .toBeInTheDocument();
});

test("login-04 is axe-clean", async () => {
  const screen = await render(<Login04Page />);
  await expect
    .element(screen.getByText("Login to your Acme Inc account").first())
    .toBeInTheDocument();
