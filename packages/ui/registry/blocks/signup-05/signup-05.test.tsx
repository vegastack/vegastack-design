/**
 * `signup-05.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Signup05Page from "./page";

test("signup-05 renders its composition", async () => {
  const screen = await render(<Signup05Page />);
  await expect
    .element(screen.getByText("Welcome to Acme Inc.").first())
    .toBeInTheDocument();
});

test("signup-05 is axe-clean", async () => {
  const screen = await render(<Signup05Page />);
  await expect
    .element(screen.getByText("Welcome to Acme Inc.").first())
    .toBeInTheDocument();
