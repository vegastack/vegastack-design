/**
 * `login-03.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Login03Page from "./page";

test("login-03 renders its composition", async () => {
  const screen = await render(<Login03Page />);
  await expect
    .element(
      screen.getByText("Login with your Apple or Google account").first(),
    )
    .toBeInTheDocument();
});

test("login-03 is axe-clean", async () => {
  const screen = await render(<Login03Page />);
  await expect
    .element(
      screen.getByText("Login with your Apple or Google account").first(),
