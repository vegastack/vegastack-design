/**
 * `sidebar-05.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar05Page from "./page";

test("sidebar-05 renders its composition", async () => {
  const screen = await render(<Sidebar05Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
});

test("sidebar-05 is axe-clean", async () => {
  const screen = await render(<Sidebar05Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
