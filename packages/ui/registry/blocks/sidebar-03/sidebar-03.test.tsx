/**
 * `sidebar-03.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar03Page from "./page";
...
test("sidebar-03 renders its composition", async () => {
  const screen = await render(<Sidebar03Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
});

test("sidebar-03 is axe-clean", async () => {
  const screen = await render(<Sidebar03Page />);
  await expect
    .element(screen.getByText("Documentation").first())
    .toBeInTheDocument();
