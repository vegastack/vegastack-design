/**
 * `sidebar-12.test.tsx` — the block's browser contract: it renders, it shows its own content,
...
import Sidebar12Page from "./page";
...
test("sidebar-12 renders its composition", async () => {
  const screen = await render(<Sidebar12Page />);
  await expect
    .element(screen.getByText("New Calendar").first())
    .toBeInTheDocument();
});

test("sidebar-12 is axe-clean", async () => {
  const screen = await render(<Sidebar12Page />);
  await expect
    .element(screen.getByText("New Calendar").first())
    .toBeInTheDocument();
