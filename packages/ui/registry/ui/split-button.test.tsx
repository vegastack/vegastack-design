...
test('composes DropdownMenuItem children via the `menu` slot', async () => {
  const onClick = vi.fn();
  const screen = await render(
    <SplitButton menu={<DropdownMenuItem onClick={onClick}>Schedule…</DropdownMenuItem>}>
      Publish
    </SplitButton>,
  );
...
test('passes variant + size through to both halves and tags the slot', async () => {
  const screen = await render(
    <SplitButton variant="destructive" size="lg" actions={[{ label: 'Force delete' }]}>
      Delete
    </SplitButton>,
  );
  ...
  await expect.element(primary).toHaveAttribute('data-slot', 'split-button-primary');
...
test('loading disables both halves', async () => {
  const onClick = vi.fn();
  const screen = await render(
    <SplitButton loading onClick={onClick} actions={[{ label: 'Save as draft' }]}>
      Save
    </SplitButton>,
  );
  await expect.element(screen.getByRole('button', { name: 'Save' })).toBeDisabled();
  await expect.element(screen.getByRole('button', { name: 'More options' })).toBeDisabled();
});
...
test('the chevron trigger icon has no motion class and is stable across a loading change (no state-driven swap in this file)', async () => {
  ...
  const trigger = screen.getByRole('button', { name: 'More options' }).element() as HTMLElement;
