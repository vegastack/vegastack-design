...
test('renders the chosen element via `as`', async () => {
  const screen = await render(
    <Subject as="p">Paragraph text</Subject>,
  );
...
test('IconText: no a11y violations (decorative icon is hidden)', async () => {
  const screen = await render(
    <TooltipProvider>
      <IconText icon={<span>★</span>} text="Accessible row" trailing={<span>3</span>} />
    </TooltipProvider>,
  );
  await expectNoA11yViolations(screen.container);
});
...
test('overflowing text is keyboard-focusable so the tooltip is reachable (tabIndex 0)', async () => {
  const long = 'A very long piece of text that will certainly overflow its tiny container';
...
test('non-overflowing text is NOT focusable (no phantom tab stop)', async () => {
  const screen = await render(<Subject>ok</Subject>);
...
test('overflowing IconText row is keyboard-focusable (tabIndex 0)', async () => {
  const long = 'An extremely long label that will overflow the constrained row width';
  const screen = await render(
    <TooltipProvider>
...
test('on a no-hover device, overflowing IconText row becomes a tap-to-toggle disclosure', async () => {
  await withNoHoverDevice(async () => {
    ...
    const row = () => screen.container.querySelector('[data-slot="icon-text"]')!;
    ...
    const label = () => screen.container.querySelector('[data-slot="icon-text-label"]')!;
