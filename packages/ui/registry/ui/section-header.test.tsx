...
test("renders the title as an h2", async () => {
  ...
  await expect
    .element(
      screen.getByRole("heading", { level: 2, name: "Ship agentic UI, fast." }),
    )
    .toBeInTheDocument();
});
...
test("renders an eyebrow with a single brand marker dot", async () => {
  const screen = await render(
    <SectionHeader eyebrow="Platform" title="Title" />,
  );
  const eyebrow = screen.container.querySelector(
    '[data-slot="section-header-eyebrow"]',
  );
  expect(eyebrow?.textContent).toContain("Platform");
  const dots = screen.container.querySelectorAll(
...
test("omits the eyebrow entirely when not provided", async () => {
  ...
  expect(
    screen.container.querySelector('[data-slot="section-header-eyebrow"]'),
  ).toBeNull();
});

test("renders an optional description", async () => {
  const screen = await render(
    <SectionHeader title="Title" description="Supporting copy." />,
  );
  await expect
    .element(screen.getByText("Supporting copy."))
    .toBeInTheDocument();
});
