...
test('applies the size data attribute', async () => {
  ...
  await expect.element(screen.getByRole('img')).toHaveAttribute('data-size', 'lg');
});

test('uses a custom label as the accessible name', async () => {
  const screen = await render(<StatusIcon status="progress" label="Deploying" />);
  await expect.element(screen.getByRole('img')).toHaveAttribute('aria-label', 'Deploying');
});

test('is decorative (aria-hidden, no img role) when label is empty', async () => {
  const screen = await render(<StatusIcon status="done" label="" data-testid="deco" />);
...
test('no a11y violations', async () => {
  const screen = await render(<StatusIcon status="progress" label="In progress" />);
