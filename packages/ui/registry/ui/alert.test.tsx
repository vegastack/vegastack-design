...
test('renders title and description content with role=alert', async () => {
  ...
  await expect.element(screen.getByText('Something happened.')).toBeInTheDocument();
});
...
test('applies variant data attribute and default variant', async () => {
  ...
  await expect.element(screen.getByRole('alert')).toHaveAttribute('data-intent', 'success');
});
...
test('self-dismisses (removes from DOM) when no onDismiss provided', async () => {
  ...
  await screen.getByRole('button', { name: 'Dismiss' }).click();
  await vi.waitFor(() => expect(screen.container.querySelector('[role="alert"]')).toBeNull());
});
