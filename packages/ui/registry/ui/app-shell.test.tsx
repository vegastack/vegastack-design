...
import { AppShell, AppShellContent, AppShellHeader, AppShellSidebar, AppShellSkeleton } from './app-shell';
import {
  SidebarContent,
  SidebarGroup,
  SidebarGroupLabel,
 ...
 */
function Demo({ contentVariant }: { contentVariant?: 'sidebar' | 'floating' | 'inset' } = {}) {
  return (
    <AppShell>
      <AppShellSidebar variant={contentVariant === 'inset' ? 'inset' : undefined}>
        <SidebarHeader>
          <span>VegaStack</span>
        </SidebarHeader>
        <SidebarContent>
...
test('the skip link is the first focusable element in the shell', async () => {
  ...
  expect(active?.textContent).toContain('Skip to content');
  expect(screen.container.contains(active)).toBe(true);
});
...
test('header renders the trigger, the actions slot, and a min-w-0 middle slot', async () => {
  const screen = await render(<Demo />);
  await expect.element(screen.getByRole('button', { name: 'Toggle sidebar' })).toBeInTheDocument();
  await expect.element(screen.getByRole('button', { name: 'New agent' })).toBeInTheDocument();

  const header = screen.container.querySelector('[data-slot="app-shell-header"]')!;
  ...
  const actionsSlot = header.querySelector('[data-slot="app-shell-header-actions"]');
...
test('header omits the actions slot entirely when no actions are passed', async () => {
  ...
  const header = screen.container.querySelector('[data-slot="app-shell-header"]')!;
  expect(header.querySelector('[data-slot="app-shell-header-actions"]')).toBeNull();
});
...
test('AppShell renders its row with data-slot="app-shell" (no extra wrapper div)', async () => {
  const screen = await render(<Demo />);
  expect(screen.container.querySelector('[data-slot="app-shell"]')).not.toBeNull();
  expect(screen.container.querySelector('[data-slot="sidebar-wrapper"]')).toBeNull();
});

test('AppShellSidebar stamps data-slot="app-shell-sidebar" and defaults aria-label', async () => {
  const screen = await render(<Demo />);
  const nav = screen.getByRole('navigation', { name: 'Main navigation' }).element();
...
test('AppShellSidebar aria-label can be overridden', async () => {
  ...
  await expect.element(screen.getByRole('navigation', { name: 'Custom nav' })).toBeInTheDocument();
});
...
test('AppShellSkeleton is decorative and renders navItemCount / statCardCount placeholders', async () => {
  const screen = await render(<AppShellSkeleton navItemCount={3} statCardCount={2} />);
  const root = screen.container.querySelector('[data-slot="app-shell-skeleton"]')!;
  expect(root.getAttribute('aria-hidden')).toBe('true');
  expect(root.getAttribute('aria-busy')).toBe('true');
  expect(root.querySelectorAll('[data-slot="sidebar-menu-skeleton"]').length).toBe(3);
...
test('AppShellSkeleton is deterministic across renders (same index -> same width class)', async () => {
  ...
  const lineA = a.container.querySelector('[data-slot="sidebar-menu-skeleton"] [data-shape="line"]');
  const lineB = b.container.querySelector('[data-slot="sidebar-menu-skeleton"] [data-shape="line"]');
