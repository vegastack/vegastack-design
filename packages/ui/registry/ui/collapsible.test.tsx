...
test('renders the trigger', async () => {
  ...
  await expect.element(screen.getByRole('button', { name: 'Show details' })).toBeInTheDocument();
});
...
test('is closed by default — trigger collapsed, no panel content', async () => {
  ...
  expect(screen.container.querySelector('[data-slot="collapsible-content"]')).toBeNull();
});
...
test('clicking the trigger opens the panel and sets data-open', async () => {
  ...
  const panel = screen.container.querySelector('[data-slot="collapsible-content"]');
...
test('disabled trigger dims via data-disabled (Base UI surfaces root-level disabled as a data attribute)', async () => {
  ...
  await expect.element(trigger).toHaveAttribute('data-disabled');
  const el = screen.container.querySelector('[data-slot="collapsible-trigger"]')!;
...
test('parts carry their data-slot attributes', async () => {
  ...
  expect(screen.container.querySelector('[data-slot="collapsible-trigger"]')).not.toBeNull();
  expect(screen.container.querySelector('[data-slot="collapsible-content"]')).not.toBeNull();
});
