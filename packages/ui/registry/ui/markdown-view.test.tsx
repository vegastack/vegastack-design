...
test('renders headings, links, code, and list elements', async () => {
  ...
  // Heading
  await expect.element(screen.getByRole('heading', { level: 2, name: 'Section' })).toBeInTheDocument();
  // Link (with safe rel)
...
test('does not render javascript: protocol links as executable', async () => {
  const screen = await render(<MarkdownView>{'[click](javascript:alert(1))'}</MarkdownView>);
...
test('keeps relative links in the current browsing context', async () => {
  const screen = await render(<MarkdownView>{'[internal](/docs/components/button)'}</MarkdownView>);
...
test('root carries the data-slot attribute', async () => {
  ...
  expect(screen.container.querySelector('[data-slot="markdown-view"]')).not.toBeNull();
});

test('renders nothing for empty / whitespace input', async () => {
  const screen = await render(<MarkdownView>{'   '}</MarkdownView>);
  expect(screen.container.querySelector('[data-slot="markdown-view"]')).toBeNull();
});

test('merges a custom className onto the root', async () => {
  const screen = await render(<MarkdownView className="custom-md">hi</MarkdownView>);
