// @vegastack input@0.2.0 sha256-Fo5fVaBFhmDNQOAjZl8iM08VzeOWBSRdvK8LEKNqPgw=
...
export interface InputProps
  extends Omit<React.ComponentProps<typeof BaseInput>, 'prefix' | 'className' | 'size'> {
  /**
 ...
 */
const fieldClasses =
  'w-full min-w-0 rounded-md border border-input bg-transparent px-3 py-1 text-base transition-colors duration-fast ease-standard outline-none ' +
 ...
 */
const groupClasses =
  'flex w-full min-w-0 items-center overflow-hidden rounded-md border border-input bg-transparent text-base transition-colors duration-fast ease-standard ' +
  'dark:bg-input/(--alpha-input) ' +
...
export function Input({
  ...
  const {
    ...
    const inputRef = React.useMemo(() => mergeRefs(ref, shakeInvalidRef), [ref, shakeInvalidRef]);
    const handleAnimationEnd: NonNullable<InputProps["onAnimationEnd"]> = React.useCallback(
      (event) => {
        onAnimationEnd?.(event);
      },
      [onAnimationEnd],
    );
    const handleStandaloneAnimationEnd: NonNullable<InputProps["onAnimationEnd"]> =
      React.useCallback(
        (event) => {
    ...
    if (prefix != null || suffix != null) {
      ...
      return (
        <div
          ...
          data-size={size}
          className={cn(groupClasses, sizeClasses[size], shakeClassName, containerClassName)}
          onAnimationEnd={shakeAnimationEnd}
        >
          {prefix != null ? (
    ...
    return (
      <BaseInput
        ...
        data-slot="input"
        data-size={size}
        className={mergeInputClassName(cn(fieldClasses, sizeClasses[size], shakeClassName), className)}
