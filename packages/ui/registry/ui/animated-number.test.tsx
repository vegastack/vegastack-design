...
function visibleText(container: Element): string | null {
  return container.querySelector('[data-slot="animated-number-value"]')?.textContent ?? null;
}

/** Reads the sr-only live-region text — the one assistive tech hears. */
function liveText(container: Element): string | null {
  return container.querySelector('[data-slot="animated-number-live"]')?.textContent ?? null;
}
...
test('renders the exact final value statically at rest — no animation on mount', async () => {
  ...
  expect(visibleText(screen.container)).toBe(new Intl.NumberFormat().format(1234));
...
test('formats a static value with currency options', async () => {
  const screen = await render(
    <AnimatedNumber value={1999.5} format={{ style: 'currency', currency: 'USD' }} />,
  );
  const expected = new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' }).format(1999.5);
...
test('tweens to the new target on a value change and settles exactly on it', async () => {
  ...
  try {
    ...
    await expect.poll(() => visibleText(screen.container), { timeout: 2000 }).toBe('100');
...
test('interrupting a tween mid-flight retargets to the NEW value with no snap-back', async () => {
  ...
  try {
    ...
    await expect.poll(() => visibleText(screen.container), { timeout: 2000 }).toBe(finalTarget);
...
test('prefers-reduced-motion renders value changes instantly, with no tween', async () => {
  ...
  const matchMediaSpy = vi.spyOn(window, 'matchMedia').mockImplementation(matchMediaMock as never);
  ...
  try {
    ...
    await expect.poll(() => visibleText(screen.container), { timeout: 500 }).toBe('100');
    expect(liveText(screen.container)).toBe('100');
  } finally {
...
test('the animated intermediate frames are formatted through the same formatter as the settled value', async () => {
  ...
  try {
    const screen = await render(
      <StatefulNumberWithFormat initial={0} target={5000} format={{ style: 'currency', currency: 'USD' }} />,
    );
    ...
    const startText = new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' }).format(0);
    const endText = new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' }).format(5000);
    let sawIntermediate = false;
    const deadline = Date.now() + 2000;
    while (Date.now() < deadline) {
      const text = visibleText(screen.container);
      if (text && text !== startText && text !== endText && text.includes('$')) {
    ...
    await expect.poll(() => visibleText(screen.container), { timeout: 2000 }).toBe(endText);
  } finally {
    cleanup();
  }
});
...
test('the visible tween text is aria-hidden and the live region announces only the settled value', async () => {
  ...
  try {
    const screen = await render(<StatefulNumber initial={0} duration="fast" />);
    const valueEl = screen.container.querySelector('[data-slot="animated-number-value"]');
    const liveEl = screen.container.querySelector('[data-slot="animated-number-live"]');
    ...
    (screen.getByRole('button', { name: 'Increment' }).element() as HTMLElement).click();
    expect(liveText(screen.container)).toBe('0');

    await expect.poll(() => liveText(screen.container), { timeout: 2000 }).toBe('100');
  } finally {
    cleanup();
  }
});
...
test('no a11y violations at rest', async () => {
  const screen = await render(<AnimatedNumber value={1234} format={{ style: 'currency', currency: 'USD' }} />);
