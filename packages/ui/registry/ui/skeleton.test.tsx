...
test('applies the shape data attribute', async () => {
  ...
  await expect.element(screen.getByTestId('sk')).toHaveAttribute('data-shape', 'circle');
});
...
test('renders a stack of `count` line placeholders', async () => {
  ...
  expect(root.element().querySelectorAll('[data-slot="skeleton-line"]')).toHaveLength(3);
});
...
test('renders exactly the skeleton prop while loading, with no extra wrapper', async () => {
  ...
  expect(screen.container.querySelector('[data-slot="skeleton-reveal-content"]')).toBeNull();
...
test('reveals the content wrapper with motion-enter-up once loading is false', async () => {
  ...
  const wrapper = screen.container.querySelector('[data-slot="skeleton-reveal-content"]');
