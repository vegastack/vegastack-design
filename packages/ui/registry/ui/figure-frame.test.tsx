...
test("renders a figure with the media children", async () => {
  ...
  await expect
    .element(screen.getByRole("img", { name: "Screenshot" }))
    .toBeInTheDocument();
...
test("omits the caption entirely when not provided", async () => {
  ...
  expect(
    screen.container.querySelector('[data-slot="figure-frame-caption"]'),
  ).toBeNull();
});
...
test("renders FIG. {number} before the caption text", async () => {
  ...
  const caption = screen.container.querySelector(
...
test("renders the caption without a figure number when omitted", async () => {
  ...
  expect(
    screen.container.querySelector('[data-slot="figure-frame-number"]'),
  ).toBeNull();
