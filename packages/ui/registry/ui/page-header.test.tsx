...
test('renders the title as an h1', async () => {
  ...
  await expect.element(heading).toHaveAttribute('data-slot', 'page-header-title');
});
...
test('renders the description', async () => {
  const screen = await render(<PageHeader title="Settings" description="Workspace preferences." />);
  await expect.element(screen.getByText('Workspace preferences.')).toBeInTheDocument();
});

test('renders the breadcrumb slot', async () => {
  const screen = await render(
    <PageHeader title="API Keys" breadcrumb={<nav aria-label="breadcrumb">Settings</nav>} />,
  );
  await expect
...
test('back button fires onBack', async () => {
  const onBack = vi.fn();
  const screen = await render(<PageHeader title="Plan" onBack={onBack} backLabel="Go back" />);
  await screen.getByRole('button', { name: 'Go back' }).click();
  expect(onBack).toHaveBeenCalledOnce();
});

test('backHref renders the back affordance as a link', async () => {
  const screen = await render(<PageHeader title="Plan" backHref="/settings" backLabel="Go back" />);
  await expect
...
test('renders the actions slot, right-aligned', async () => {
  const screen = await render(
    <PageHeader title="Spaces" actions={<button type="button">New space</button>} />,
  );
  await expect
    .element(screen.getByRole('button', { name: 'New space' }))
    .toBeInTheDocument();
  const actions = screen.container.querySelector('[data-slot="page-header-actions"]')!;
...
test('title row lets the actions wrap below the title at narrow widths (no crushed h1)', async () => {
  ...
  const screen = await render(
    <PageHeader title="Quarterly review" actions={<button type="button">Export</button>} />,
  );
  ...
  const actions = screen.container.querySelector('[data-slot="page-header-actions"]') as HTMLElement;
...
test('favorite star is controlled when active is provided', async () => {
  const onToggle = vi.fn();
  const screen = await render(
    <PageHeader title="Doc" favorite={{ active: true, label: 'Favorite', onToggle }} />,
  );
  const star = screen.getByRole('button', { name: 'Favorite' });
  await expect.element(star).toHaveAttribute('aria-pressed', 'true');
  await star.click();
...
test('omits the back, favorite, and actions slots when not configured', async () => {
  ...
  expect(screen.container.querySelector('[data-slot="page-header-back"]')).toBeNull();
  expect(screen.container.querySelector('[data-slot="page-header-favorite"]')).toBeNull();
  expect(screen.container.querySelector('[data-slot="page-header-actions"]')).toBeNull();
});
...
const TITLE_CLIP =
  '[data-slot="truncated-text"] { display: block; overflow: hidden; white-space: nowrap; max-width: 48px; }';
...
test('an overlong title truncates via TruncatedText and is keyboard-focusable', async () => {
  const long = 'A workspace title so long it will certainly overflow the constrained header row';
  const screen = await render(
    <TooltipProvider>
