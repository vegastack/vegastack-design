...
test('renders a past instant as "ago" copy', async () => {
  ...
  const screen = await render(<RelativeTime date={date} now={NOW} title={false} />);
...
test('unitStyle="narrow" renders the compact form ("2h ago")', async () => {
  ...
  const screen = await render(
    <RelativeTime date={date} now={NOW} title={false} unitStyle="narrow" locale="en" />,
  );
...
test('collapses a sub-minute delta to "now"', async () => {
  const screen = await render(<RelativeTime date={new Date(ms(-5_000))} now={NOW} title={false} />);
...
test('accepts an ISO string and an epoch-number date', async () => {
  ...
  const num = await render(<RelativeTime date={ms(-60_000)} now={NOW} title={false} />);
...
test('day mode labels adjacent days and sets data-mode', async () => {
  const yesterday = await render(
    <RelativeTime date={new Date(ms(-86_400_000))} now={NOW} mode="day" title={false} />,
  );
...
test('day mode falls back to an absolute date for distant days', async () => {
  ...
  const sameYear = await render(
    <RelativeTime date={new Date(Date.UTC(2026, 2, 15, 12))} now={NOW} mode="day" title={false} />,
  );
  ...
  const otherYear = await render(
    <RelativeTime date={new Date(Date.UTC(2024, 2, 15, 12))} now={NOW} mode="day" title={false} />,
  );
  await expect.element(otherYear.getByText('March 15, 2024')).toBeInTheDocument();
});
...
test('accepts a custom tooltip label', async () => {
  const screen = await render(
    <TooltipProvider>
      <RelativeTime date={new Date(ms(-3_600_000))} now={NOW} title="Created at launch" />
    </TooltipProvider>,
  );
  await userEvent.tab();
  await expect.element(screen.getByRole('tooltip')).toHaveTextContent('Created at launch');
});
...
test('forwards ref to the <time> element', async () => {
  ...
  await render(<RelativeTime ref={ref} date={new Date(ms(-3_600_000))} now={NOW} title={false} />);
