...
- **Theme engine** — `next-themes` (0.4.6): the class/attribute theme switcher, the storage and system-preference listener, and the anti-FOUC inline script. Approved by MK 2026-09-07 (audit `2026-09-07-system-audit`, decision **D30**). It is **mounted** in exactly one registry item — `provider`, the sanctioned single app-root wrapper. One other item reads it: `sonner` calls `useTheme()` to resolve the toaster's colour scheme. That is a read of the provider already on the page, never a second mount; a swap changes `provider` plus that one `useTheme()` call.
...
- **Registry items: 563** — 114 components · 439 animated icons · 9 hooks (`use-animation-replay`, `use-announcer`, `use-drag-reorder`, `use-file-drop`, `use-inline-edit`, `use-list-nav`, `use-media-query`, `use-mobile`, `use-platform`) · 1 block (`dashboard-01`)
- Contract SHA-256: `b21b1140f199777b9d42505b601436d027f758082fe14b2277b6bd19e4679d9c`
