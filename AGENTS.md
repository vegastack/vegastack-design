...
- **Renderer / behavior engines** — `react-resizable-panels`, `recharts`, `motion` and `tiptap`. Each is named per-component in `packages/ui/registry.json`. These render or animate; they do not own interaction semantics, which is why they are a narrower class than the primitive exception above.
...
- **Registry items: 596** — 116 components · 467 animated icons · 10 hooks (`use-animation-replay`, `use-announcer`, `use-drag-reorder`, `use-file-drop`, `use-inline-edit`, `use-list-nav`, `use-media-query`, `use-mobile`, `use-overflow`, `use-platform`) · 1 block (`dashboard-01`) · 2 libs (`geo-data`, `drag-item`)
- Contract SHA-256: `8e32b22a6533de57198e4ededf5b74f5cf47e328f6a8568537820b2b68824095`
