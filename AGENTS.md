...
- **Headless primitives** — non-Base-UI packages that own a behavioural core (interaction semantics or the state machine under them) but render nothing. Exactly nine, each isolated behind one registry item so an engine swap touches one file:
  ...
  - `@dnd-kit/{core,modifiers,sortable,utilities}` v6/v9/v10/v3 — the row-drag engine inside upstream's `dashboard-01` **block**: the sensors, the collision detection, the sortable context and the transform maths behind its draggable data-table rows. The four packages are ONE engine identity, exactly as `-hitbox` folds into the pragmatic one. It is the first entry here whose one file is a BLOCK rather than a component (`packages/ui/registry/blocks/dashboard-01/components/data-table.tsx`), and that is the narrower claim it makes: a block is copy-once, so a consumer who never installs `dashboard-01` never installs this. It does not displace `@atlaskit/pragmatic-drag-and-drop`, which remains the engine behind `use-drag-reorder` and therefore behind every drag surface the SYSTEM owns. Pre-approved with upstream's whole dependency set by MK 2026-09-18 (`docs/plans/2026-09-18-shadcn-reset/implementation.md` § 9, decision **DOC-7**), adopted in Batch 8.
...
- **Registry items: 691** — 111 components · 467 animated icons · 11 hooks (`use-animation-replay`, `use-announcer`, `use-drag-reorder`, `use-file-drop`, `use-inline-edit`, `use-list-nav`, `use-media-query`, `use-mobile`, `use-modal-inert`, `use-overflow`, `use-platform`) · 32 blocks · 68 chart blocks · 2 libs (`geo-data`, `drag-item`)
- Contract SHA-256: `42b9da70d636a6471e5bfcbf75f6f6197001514a8715e3b76b2ed600c6230c7b`
