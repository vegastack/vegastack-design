...
**Not everything generated is committed.** Two contract-derived files — the home catalog and the
animated-icon gallery — are **build outputs**: `.gitignore`d, listed in
`tooling/lib/derived-build-outputs.mjs`, and written by `apps/docs`'s `prepare:content` (which
already runs before its `build`, `dev`, `lint`, and `typecheck`) or on demand by the few tooling
readers that run before any build. Both belong to `apps/docs`; `packages/ui` has no build output
and therefore no `prepare:content` task at all. Never stage one; `pnpm design:derived:check` deliberately ignores them, and
...
pnpm verify:release           # deploy   BOTH docs matrices · links · docs shell · registry · consume · 3 engines
...
| `pnpm verify:release` — BOTH docs matrices, links, docs-shell contracts (+ self-test), registry idempotency, consume, three engines | `deploy.yml`, before `build-sign-deploy`        |
...
- Contract SHA-256: `24980d0c0dae884ce8e6a64ea986c0d80f022e65e71c66deb13886b997fec7e0`
