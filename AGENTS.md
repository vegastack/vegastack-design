...
- **No focus rings anywhere, Tabs included** (FOC-13, MK 2026-09-25) — `base.css` owns the one focus cue: a
  subtle `accent`/50 background tint on every focusable element, text entry included (a bordered
  field group — `data-field-group` — wears it on the group); the contenteditable document editor
  (TextEdit) is caret-only; forced colours restore the outline.
- **A border never changes colour on focus or while active** (FOC-14, MK 2026-09-26) — every
  control, field group and wrapper keeps its resting `border-border`/`border-input` in every state:
  no `focus:`/`focus-visible:`/`focus-within:`/`data-focused:`/`data-popup-open:` border change.
  The one exception is invalid, whose destructive border holds focused or not (no `not-focus:`
  guard). `design-lint`'s `no-focus-border` rejects a focus border class, and the geometry lane's
  focus sweep fails any control whose border colour moves on focus. `design-lint`'s `no-focus-ring-glow` rejects `ring-3`,
...
- **Accessibility** — WCAG 2.2 AA while preserving every existing 2.1 assertion; visible
  `:focus-visible` background tint (text entry too; never a border change, FOC-14), enforced per control by the
...
- **Registry items: 691** — 128 components · 467 animated icons · 13 hooks (`use-animation-replay`, `use-announcer`, `use-async-search`, `use-drag-reorder`, `use-file-drop`, `use-inline-edit`, `use-list-nav`, `use-media-query`, `use-mobile`, `use-modal-inert`, `use-overflow`, `use-platform`, `use-tabs-swipe`) · 11 blocks · 68 chart blocks · 4 libs (`date-time`, `geo-data`, `emoji-data`, `drag-item`)
- Contract SHA-256: `97bc1c585ffdf40aa6ba4742dbf4aa67be3c84d28eaff6591d8f36de0afa27e0`
