...
- **Dropzone (useFileDrop · Dropzone)** — file ingestion is drop + paste + browse, always all three; the hidden real `<input type="file">` carries the accessible name. Rejections are typed and non-destructive (too-many/too-large/wrong-type stay listed with readable reasons, sub-AA copy banned); `data-dragging`/`data-drag-invalid` drive the only visual states — a dashed border tint, no scale theatrics. The hook prevents the window-level drop default so a missed drop never navigates away from unsaved work.
