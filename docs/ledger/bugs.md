...
## 2026-09-22 — the Version Packages PR could not install itself

- **Symptom.** The generated Version Packages PR for 0.12.0 failed its required `PR quality` run at
  the very first step, before any gate executed:
  `[ERR_PNPM_OUTDATED_LOCKFILE] ... @vegastack/design-tokens (lockfile: ^0.5.0, manifest: ^0.7.0)`.
  Every source PR feeding the release was green, and the generated diff was correct — the release
  simply could not be installed.
- **Root cause.** `@vegastack/design` declares `@vegastack/design-tokens` by **semver range**, not
  `workspace:*`, and `pnpm-lock.yaml` records that range as a `specifier`. `version-sync` rewrites
  the range when a bump moves the dependency out of it, but `version-packages` —
  `changelog-assemble → changeset version → version-sync → sync-changelog` — never regenerated the
  lockfile afterwards. So the generated branch committed a manifest and a lockfile that disagreed,
  and `--frozen-lockfile` is designed to refuse exactly that.
- **Why every previous release passed.** The rewrite only happens when the new version leaves the
  pinned `^` range. The two packages are `linked` in the changesets config, but linking only aligns
  them on releases that bump BOTH; the last few bumped `design` alone, so it sat at 0.6.1 against
  `design-tokens` 0.5.0 with `^0.5.0` still satisfied. 0.12.0 is the first release to move
  `design-tokens` (0.5.0 → 0.7.0), so it is the first to rewrite the range at all. **The defect was
  latent for every release since the pin was written, and dormant is not absent.**
- **Systemic fix.** `version-packages` now runs `pnpm install --lockfile-only --ignore-scripts`
  between `version-sync` and `sync-changelog`, so the manifest and the lockfile are regenerated by
  the same command that desynchronised them, and `changesets/action` commits both together.
  `linkWorkspacePackages: true` means the pair resolves locally — the refresh needs no network and
  does not require the new `design-tokens` to exist on npm, which it does not at that moment. Why it
  is there is recorded in `docs/RELEASING.md` § Changesets and changelog, because a step whose
  purpose is invisible is a step someone deletes.
- **No new gate.** The check that should have caught this is the Version PR's own required run, and
  it did — it failed loudly, at the first step, naming both versions. The bug was a missing step in
  the pipeline, not a missing assertion about it; adding a second place to notice would be red tape.
- **The generalisable rule.** **Any command that edits a `package.json` dependency range must
  regenerate the lockfile in the same breath.** A lockfile is not a cache of a manifest, it is a
  second record of the same fact, and a release pipeline that writes one without the other produces
  an artifact that cannot be installed — discovered at the least convenient moment, on generated
  output, with the source PRs already merged.
- **Sweep.** `version-sync.mjs` is the only tool in the repository that rewrites a dependency range,
