...
| **Switch** | `h-4/w-7` | `h-5/w-9` | `h-6/w-11` | `switch.tsx:29`. Own scale (16/20/24 track height) — appropriate for a track, but note the `sm/default/lg` *names* map to different literal pixels than Button/Select's `sm/default/lg`. Purely a naming-collision risk for anyone assuming one universal size vocabulary. |
...
**Overall focus-system verdict:** the *global* mechanism (one outline token, offset 1, border-darken
...
| **Command** (palette) | *(no direct `shadow-*` in `command.tsx`; it is always composed inside a `Dialog`/`Popover` wrapper in this registry, so it inherits `shadow-overlay` from the host)* | Overlay (inherited) | Consistent — but only by composition, not an intrinsic property; a bare `<Command>` used outside Dialog/Popover would render with **no shadow at all**, worth a doc note. |
...
- Switch's `sm/default/lg` (`16/20/24`) and Button/Select's `sm/default/lg` (`28/32/40`) are two
  unrelated pixel scales sharing the same size-tier *names* — no bug today, but a naming trap for
...
- **No `font-bold` anywhere** in the registry — confirmed via full-text grep. The heaviest weight in
  ...
  *exceeds* the written rule ("never 600+") — the codebase never even reaches for 600, only 400/500.
...
- This isn't cosmetic-only: raw Tailwind sizes are **not** remapped to the design-system scale
  ...
  spec's 14px "default body" — it silently collides with the *`text-lg`* token's pixel size instead.
...
- `markdown-view.tsx` and `text-edit.tsx` mirror each other's heading treatment 1:1 (both cap at
  ...
  `text-h1..h4`) — consistent with *each other* (by design, per their own doc comments), but both are
...
- **Card padding does not match its own spec.** `design.md` prose (§Layout) states "Cards use 20px
  ...
  the *default* (not "compact") with a `sm` variant at `py-3`/`px-3` = 12px, and **no 20px or 32px
...
- Dialog/AlertDialog use `p-5` (20px, `dialog.tsx:30`) matching the *recipe table's* dialog spec
...
- **Button's `destructive` variant does not implement `design.md`'s `button-destructive` recipe.**
  ...
  shipped `Button` component has no such variant: its `destructive` variant is actually the *soft*
...
- `size-4` (16px) is the dominant in-control icon size (65 uses) matching `design.md`'s "default 16"
  ...
  variants (`button.tsx:45,49`) — consistent *within* that undocumented `xs` tier, but the tier itself
...
- Chevron sizing itself is not asserted via an explicit `size-*` class in `Select`
  ...
  parent selector) — functionally converges on the same 16px, but the *mechanism* differs
...
- **But the pattern itself is inconsistently scoped.** `dark:bg-input/30` (a translucent dark-mode
  tint layered *on top of* the already-dark-aware `input`/`secondary` token) is applied to Input,
  ...
  worth a design decision on whether form controls genuinely need an *additional* dark treatment other
