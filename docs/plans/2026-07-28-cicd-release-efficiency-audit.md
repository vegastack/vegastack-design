...
| remediation | `8c5e4ff4`, `31a32ee8`, `d1655e83`, `f12f24ff`, `78db32a0`, `9e1211ab`, `5b87b1cf`, `f653e49d`, `9546a03b`, `fa33e08a`, `f47a7429`; current docs, machine authority, workflow/CLI verifiers, runner diagnostics, deploy probe, VRT/package harness | stale topology/count/timing/provenance/preflight/retry/terminal/browser-location/command claims fail semantically; diagnostics use supported package commands/wrappers and structured nonempty terminal reports; derived authority digests are regenerated                | active correctness surfaces; each commit independently revertible, but reverting reopens its recorded finding                        | operator verifier covers 32 surfaces, 40 semantic fixtures and 9 executable help surfaces; workflow verifier has 52 mutations |
...
| VRT                        | scroll-driven TOC and JS-regex syntax scopes produced false changes                            | first comparison: base 940 tests/972 images in 23.5m; working 883 unchanged/57 changed in 19.6m. Inspection found 56 syntax-scope changes and one TOC state race. Oniguruma then produced 77,338 identical Shiki blocks across consecutive builds; TOC state is stabilized only in VRT | measured local paired observations, `n=1` each             | nondeterminism fixed; final human visual acceptance still required |
...
| zero current-instruction drift                                      | known contradictions           | 32 current surfaces pass; 40 semantic mutations and machine-derived 108/864/5 counts | executable local proof                      | met locally                                    |
...
| VRT human review / MK                    | final-tree report and intentional Oniguruma syntax-colour change review                                                      | inspect `.vrt-review/report.json` and every non-unchanged image before push approval                                        | revert `f12f24ff` only if the intentional highlighting change is rejected; do not waive unexplained pixels |
