...
7. **Compare per component** (status by HASH, version is display-only — avoids false positives when the global version bumps but a component's content didn't change):
   ...
   - **No local-edit ("✎") status** — it cannot be derived from the static header alone (the header is the *published* hash, not a hash of the current file body). Footer caveat instead: "`--overwrite` replaces files; if you've customized a component, `git diff` / `verify --post-write` before applying."
8. **Output:**
