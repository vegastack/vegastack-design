...
4. The `detail/*` files hold the copy-paste configs. The phase steps below tell you *which* detail file and *in what order*.
...
| **Primitives** | `@base-ui/react` | `1.6.0` *(NOT `@base-ui-components/react` — deprecated/renamed)* |
...
| **DL2** | Docs site uses **`fumadocs-ui` (Radix-internal, `fd-*` namespaced)**; it coexists with our Base UI *showcased* components — no collision. | Fumadocs internals are namespaced; verified in `references/fumadocs/packages/radix-ui`. |
...
### P1 — Foundations + showcase shell  *(the pre-checkpoint heavy lift)*
