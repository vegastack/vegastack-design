...
> **Historical record — no current authority.** This plan was approved and implemented. Its
> “awaiting approval” status and pre-overhaul measurements are preserved only as decision history.
> The root `design.md` and its sync gate are the current design doctrine.
...
We use shadcn registry + Base UI. In **shadcn**, `--primary` = the main action colour and `--accent` = the _neutral hover/selected_ fill. Our mapping:
