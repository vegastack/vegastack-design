...
> **Historical record — no current authority.** The icon mirror described here has been implemented
> and subsequently reorganized. Its package names, item counts, source layout, and “not implemented”
> status are stale by design. Use the current registry, component contract, and `AGENTS.md`.

**Goal:** Support _all_ icons from the three sanctioned sources through `@vegastack/icons`. Build local, uncommitted.
