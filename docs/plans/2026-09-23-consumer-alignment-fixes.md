...
- New decision row **BRD-1 → ours**: "Cards and floating surfaces draw a 1px `border border-border`; no `ring-*` box-shadow outline." Regenerate `decisions.json` (`node tooling/upstream/verify-parity.mjs --sync-decisions`), update `exception-map.json`. **Extension (MK, 23-09-2026, review round 1):** the floating sidebar's `ring-1 ring-sidebar-border` is the same outline, so `sidebar` joins BRD-1 in `exception-map.json` and draws `border border-sidebar-border`; the register row records the extension, and `no-surface-ring` now rejects every 1px ring width and hairline ink, not only `ring-foreground/…`.
...
- Destructive menu items use the fill colour as text on a 10% tint (3.99:1, below AA by our own `foundations/colors` measure): `dropdown-menu.tsx:109`, `context-menu.tsx:118`, `menubar.tsx:106` → `text-destructive-text`. Same for `questionnaire.tsx:209`, `attachment.tsx:58` (contradicts A11Y-13 in its own file at :128). **Correction (23-09-2026):** `attachment.tsx:58` is not changed. It is the error ICON, not text: the A11Y-13 row measured it at 3.973:1 on its tint against the 3:1 non-text floor, so it stays upstream verbatim, and `:128` (the error description, which A11Y-13 does move) is not a contradiction.
