...
| D2 | **Apply v2 *character* onto the repo's existing token architecture, minimal structural disruption.** Deliver the signature v2 changes faithfully (warm neutrals; `brand`=purple; `ring`=`primary`; charcoal `primary`=new neutral.700; sizing 28/32/40; selected=brand; charts→8). Keep the repo's sound bits where v2 barely differs (status/info hues already AA-passing & close to v2; sidebar-* structure). | Forcing exact v2 hexes onto a different ramp is high-risk for marginal gain; the 59 components + gates already depend on the existing architecture. |
