...
| ~~8.5~~ | ~~Fix a stale claim in `table.mdx`~~ — **WITHDRAWN 2026-07-27**: the repo is already correct. The sentence covers only the seven SUB-parts and `TableProps` is documented directly above it; the stale text exists only on the **deployed** docs site. See the §12 retraction | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | none                                                                                                                                                                |
...
> **RETRACTED 2026-07-27 — four earlier entries were false, and the root cause matters.** An earlier draft listed a
> `lucide-react` pin inconsistency (`^1.20.0` × 35 / `^0.525.0` × 6 against an installed `^1.24.0`)
> two stale lines in `docs/ledger/authoring-guide.md` (`:65` "^1.21.0", `:68` "ten group
> headings"), and a stale API claim in `table.mdx`. **All four are wrong.** Verified against the committed tree, not the working copy:
> `git show HEAD:packages/ui/registry.json | grep -o '"lucide-react@[^"]*"' | sort | uniq -c` returns
> **41 × `^1.24.0`** and nothing else, and `git show HEAD:docs/ledger/authoring-guide.md` already
> says `^1.24.0` and "the **eleven** group headings in use". `table.mdx`'s "add no props of their
> own" sentence lists only the seven SUB-parts — `Table` itself is excluded, and an
> `<AutoTypeTable name="TableProps" />` documents `grid`/`headerTone`/`density` directly above it.
>
> **Root cause, worth internalising:** those claims were checked against `llms-full.txt` fetched from
> the deployed docs site, which **lags this repo**. The deployed Table page really does carry the old
> text; the repo fixed it and has not been deployed. Two independent review agents inherited that
> stale source and both "confirmed" it. **Verify against the repo (`git show HEAD:<path>`), never
> against the published site or its `llms-full.txt`** — this is the truth hierarchy in `AGENTS.md`
> applied to an external artifact. **Do not "fix" any of them —
> there is nothing to fix, and an edit would introduce the very inconsistency it claims to remove.**
> The general rule they were meant to carry is still sound and is stated in §5.1: take a dependency
> pin from `packages/ui/package.json`, never from a neighbouring registry item, because
> `verify-registry-deps.mjs` genuinely does not check version ranges.

1. **The focus-indicator contract check cannot fail** (`docs/ledger/bugs.md`, 2026-07-25) — it passes
...
2. **The `data-list.tsx` hit-area invariant is documented but unguarded** (§4) — no test would fail
   if a cell gained `overflow-hidden`.
3. **The consume path has a silent-failure mode that cost a real consumer an hour** — see §12.1
   below; it is the most valuable finding in this list because it is the one a new consumer hits
   first.

### 12.1 Consumer finding — a missing `postcss.config.mjs` fails in two different, misleading ways

Found while scaffolding VegaStack CRM (a pnpm workspace) against `@vegastack/design@0.2.0`.

`preset.css` opens with `@import "tailwindcss"; @import "tw-animate-css";`. If the consuming app has
no `@tailwindcss/postcss` plugin configured, **Tailwind's engine never runs**, and what happens next
depends on the bundler:

- **Turbopack** (the Next 16 default) fails the build with
  `Module not found: Can't resolve 'tw-animate-css'` — pointing at a _dependency_ that is installed
  and correctly symlinked. The error is actively misleading; it cost an hour and produced a wrong
  diagnosis (an incorrect "Turbopack can't resolve the `style` export condition" conclusion, and a
  needless `--webpack` pin, both since reverted).
- **webpack** resolves the import and the build **succeeds** — but the token theme is literal CSS so
  it lands, while **zero utility classes are generated**. A consumer sees correct colours and fonts
  with no spacing, layout or typography utilities, which reads as "the design system is broken".

The quickstart never mentions `postcss.config.mjs` because its step 1 runs
`pnpm create next-app --tailwind`, which generates it silently. Any consumer who scaffolds by hand,
adds the design system to an existing app, or builds a monorepo hits this immediately.

**Fix applied:** a Setup-check step in the quickstart plus a Troubleshooting entry keyed on both
symptoms (§8.10, §8.11). **Fix recommended but not applied:** have `vegastack-design` (or a
`doctor` subcommand) assert the plugin is configured, since a build that succeeds with no utilities
is exactly the class of failure a human will not attribute correctly.
