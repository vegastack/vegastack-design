...
- **Part B — Coverage fixes** so every component demonstrates *all* its variants / props / states, with the **"full matrix where useful"** bar you selected, plus corrects docs that are out of date.
...
- **0 missing pages, 0 fully-missing previews** — the problem is *thin* coverage, not absent docs.
...
5. **`select` `multiple` ambiguity** — `Select` has a `multiple` type param, yet its Do/Don't says "never pick multiple values inline." This is a *product* decision, not a doc bug: either (a) document multi-select and drop the contradictory Don't, or (b) mark `multiple` intentionally unsupported and note it. **Needs your intent.**
...
- Where **two axes interact meaningfully** (e.g. `badge` variant×color, `switch` size×on/off, `status-icon` status×size, `button` variant×size), add a **combination grid** — but only where it reveals a real visual difference. Per-component matrix recommendations are in `04`; cases where a matrix adds *no* signal are explicitly marked "skip."
