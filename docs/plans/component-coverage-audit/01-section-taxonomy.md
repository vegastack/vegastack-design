...
> Primary interactive triggers — what a consumer reaches for first. `toggle`/`toggle-group` share `toggleVariants` and sit next to Button. *(Boundary: `toggle-group` is selection-like — see Decision #3.)*
...
> Rendering values/records. *(Boundary: `filter-bar` — see Decision #4.)*
...
> The existing `---Communication---` group. *(Boundary: `marker` — see Decision #2.)*
...
### `index.mdx` (landing)
- **[P0 PROSE]** `index.mdx:6` — "**64 components**" → **68**. *(Verified: 68 mdx pages, 68 non-divider meta entries.)*
