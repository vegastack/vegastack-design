...
Coverage % is the auditing subagent's estimate of _how much of the real API surface is demonstrated in live previews_. Severity: ⛔ Major · ◑ Moderate · ✓ Strong (polish only). Effort: S/M/L. Section = proposed taxonomy (`01`).
...
> **Action:** add a single explicitly-labeled "live" example for the headline behaviors that _can't_ be frozen (relative-time refresh, sonner toasts); trim or snippet the rest.
