...
- [PROSE] **STALE/CONTRADICTORY:** mdx:27 says "The trigger is a **round swatch**" and mdx:82-area context implies round, but canonical:156 comment explicitly states "Trigger is a control → `rounded-md` (Button's native radius); **no `rounded-full`**" and the preview comment (preview:10) says "click the `rounded-md` trigger". The mdx Usage prose calling the trigger "round" is stale — the trigger is `rounded-md`/`rounded-sm`, only the in-grid swatches are `rounded-full`. QUOTE (mdx:27): _"The trigger is a round swatch showing the current selection"_.
...
- [PROSE] Anatomy code block (mdx:47–58) lists column fields `{ key, header, render?, sortable?, align?, className?, headerClassName? }` but **omits `interactive?`** (canonical:71) — the column shape in the doc is incomplete. QUOTE (mdx:48): _"columns={[{ key, header, render?, sortable?, align?, className?, headerClassName? }]}"_.
...
- coverage: ~85% (sizes + combos + groups all shown, full API tables; the only real miss is a _live_ `os="other"` preview)
- effort: S
