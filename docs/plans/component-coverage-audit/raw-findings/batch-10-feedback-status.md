...
- [VARIANT] `card` shape is demonstrated only _inside_ the `skeletonShapes` stack and the composed card — it reads fine, OK. But `count` (the headline prop) has NO standalone live preview: the "Line count" section (mdx:49-55) is a bare code block `<Skeleton count={4} />` with no `<ComponentPreview>`. Add a live count preview.
