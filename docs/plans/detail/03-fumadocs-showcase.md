...
const generator = createGenerator({ cache: createFileSystemGeneratorCache('.next/fumadocs-typescript') });
...
export default async function Page(props: PageProps<'/[[...slug]]'>) {
  ...
  const PreviewComp = page.data.preview && (page.data.preview in Preview)
    ? Preview[page.data.preview as keyof typeof Preview] : null;
  return (
...
export default function Layout({ children }: LayoutProps<'/'>) {
  return (
    <html lang="en" suppressHydrationWarning>
      <body className="flex flex-col min-h-screen"><Provider>{children}</Provider></body>
    </html>
  );
}
```
...
```json
{ "compilerOptions": { "paths": { "@/*": ["./*"], "collections/*": ["./.source/*"] }, "moduleResolution": "bundler", "jsx": "react-jsx", "plugins": [{ "name": "next" }] } }
```

`package.json` scripts:
```json
{ "scripts": { "build": "next build", "dev": "next dev", "start": "serve out", "types:check": "fumadocs-mdx && next typegen && tsc --noEmit" } }
```
...
export function Wrapper(props: HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={cn('rounded-lg border border-fd-border p-6 not-prose flex items-center justify-center', props.className)}>{props.children}</div>;
}
```
...
export function button(): ReactNode {
  return <Wrapper><Button>Save</Button></Wrapper>;
}
...
export async function ComponentPreview({ name, file }: { name: string; file: string }) {
  const Comp = Preview[name as keyof typeof Preview] as () => React.ReactNode;
  const source = readFileSync(file, 'utf8');
  const rendered = await highlight(source, { lang: 'tsx', themes: { light: 'github-light', dark: 'github-dark' }, components: { pre: Pre } });
  return (
    <Tabs items={['Preview', 'Code']}>
      <Tab value="Preview"><Comp /></Tab>
      <Tab value="Code"><CodeBlock>{rendered}</CodeBlock></Tab>
    </Tabs>
  );
}
```
...
```mdx
<AutoTypeTable path="../../packages/ui/src/components/button/button.tsx" name="ButtonProps" />
```
...
function initOrama() { return create({ schema: { _: 'string' }, language: 'english' }); }

export default function DefaultSearchDialog(props: SharedProps) {
  const { locale } = useI18n();
  const { search, setSearch, query } = useDocsSearch({ client: oramaStaticClient({ initOrama, locale }) });
  return (
...
export const revalidate = false;
export function GET() { return new Response(llms(source).index()); }
```
...
html > body[data-scroll-locked] { margin-right: 0px !important; --removed-body-scroll-bar-size: 0px !important; }
```
