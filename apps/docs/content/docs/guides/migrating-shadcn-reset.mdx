...
- **Never edit a file under `components/ui/`.** Those are re-pulled in stage 4 and `--overwrite`
  destroys local edits. Fix call sites, or wrap.
...
- **A toast fired from inside an open dialog renders behind the scrim.** There is one `z-50` band and
  ...
  mount your toaster after the dialog.
...
<Callout type="info">
  ...
  `tracking-*` you were using to compensate, and that `uppercase` is no longer
  used anywhere in the system. See [Typography](/docs/foundations/typography).
</Callout>
...
2. `text-h2` → `text-xl font-semibold tracking-tight`.
