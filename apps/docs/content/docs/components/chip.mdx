...
`active` promotes the neutral chip from its rest fill (`muted`) to the selection rung
(`accent`) — the same step every selected surface in the system takes. It is ignored for a
