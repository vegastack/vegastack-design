...
> **The live demo below uses ⌘J, not ⌘K.** This documentation site already binds ⌘K to its own
> search dialog, and two page-level handlers on one chord open both dialogs at once. Your app
> binds ⌘K, exactly as the snippet shows.
...
<TypeTable
  type={{
    items: {
      ...
      description:
        "Flat items, or groups of items. Drives filtering and CommandEmpty.",
    },
    children: {
...
<TypeTable
  type={{
    value: {
      type: "unknown",
      description:
        "The item's value, matched against Command's items/selection.",
    },
    disabled: {
      type: "boolean",
    ...
    onSelect: {
      type: "(value: unknown) => void",
      description:
        "Called when the item is activated (click, or Enter while highlighted).",
    },
    children: {
      type: "React.ReactNode",
...
<ApiTable
  path="../../packages/ui/registry/ui/command.tsx"
  name="CommandFooterProps"
/>
...
<ComponentChangelog name="command" />
