...
- The trigger is a native `<button>`, so it keeps the browser&rsquo;s focus outline and never sets `outline: none`. Its pointer affordance is the row wash (`surface-2` on hover, `surface-3` pressed) — a disclosure is not a link, so it never underlines on hover.
...
<ComponentChangelog name="collapsible" />
