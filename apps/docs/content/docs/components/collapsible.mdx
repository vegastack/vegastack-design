...
- The trigger is a native `<button>`, so it keeps the browser&rsquo;s focus outline and never sets `outline: none`. Its pointer affordance is the row wash (`accent` on hover, `accent` pressed) — a disclosure is not a link, so it never underlines on hover.
