...
(`rounded-(rounded-[2px])`), and a mono-uppercase label (the brand voice layer). Reserve it for
...
- On `:focus-visible` every variant keeps the design system's global 2px focus ring (`outline-ring`) — `Button` sets no `outline: none` of its own, so the single global focus rule applies. The `outline` variant additionally re-colors its border with the `ring` token (`focus-visible:border-ring/(70%)`).
