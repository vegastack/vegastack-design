...
- Focus is trapped inside the panel while open and restored to the trigger on close. For the default
  modal, VegaStack mirrors Base UI's live outside-tree markers to native `inert`, shared with Dialog
  and AlertDialog through reference-counted ownership. `modal={false}` and
  `modal="trap-focus"` retain their Base UI outside-interaction contracts. Page scroll is locked.
