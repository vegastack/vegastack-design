...
re-colors the border with the `destructive` token
(`aria-invalid:border-destructive/(70%)`) — pair it with a visible error
...
`has-aria-invalid:border-destructive/(70%)` marks it invalid. As on a bare
...
<ApiTable path="../../packages/ui/registry/ui/input.tsx" name="InputProps" />
...
- On focus the field recolors its border with the `ring` token (`focus:border-ring/(70%)`) instead of removing the outline — never `outline: none` with no replacement affordance. `Textarea` and `OTPInput` use the identical border tint, so text-entry focus reads the same across all three.
