...
audience: public
registry: icon-button
status: stable
since: 0.1.0
a11y: native button (aria-label required)
preview: iconButton
---

## Install

<InstallSteps name="icon-button" />
...
Since Batch 2 of the shadcn reset `Button` has one axis, so a destructive icon action is
`variant="destructive"` — a soft tint inked with `destructive-text`, never a solid red square.

<ComponentPreview
  name="iconButtonDestructive"
  file="components/preview/icon-button.tsx"
/>
