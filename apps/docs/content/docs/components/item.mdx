...
<AutoTypeTable path="../../packages/ui/registry/ui/item.tsx" name="ItemMediaProps" />

### ItemSeparator

<AutoTypeTable path="../../packages/ui/registry/ui/item.tsx" name="ItemSeparatorProps" />
...
- An interactive `Item` (`render={<a />}` or `render={<button />}`) keeps its native `link` /
  ...
  rows in a group, rather than as the *only* children of a `role="list"` `ItemGroup`.
