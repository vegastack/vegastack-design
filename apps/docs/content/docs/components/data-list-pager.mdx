...
so a late font cannot push _Next_ outside.
...
Omit `onPageSizeChange` for a fixed page size: the pager shows the range and the page controls, and
no rows-per-page chooser.

<ComponentPreview
  name="dataListPagerFixedSize"
  file="components/preview/data-list-pager.tsx"
/>
...
- The page list is a `navigation` landmark named _Pagination_. Its controls are **buttons** — they
  change the page in place rather than navigating — each page named _Go to page N_, with the current
