...
`EmojiPicker` composes [`Popover`](/docs/components/popover) (trigger + floating panel), a sticky
search row of its own — a leading glyph and a bare text field, because a bordered input inside a
bordered popup would nest two borders — and a scrollable grid of icon
[`Button`](/docs/components/button)s.
