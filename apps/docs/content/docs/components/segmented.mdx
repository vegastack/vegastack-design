---
title: Segmented
description: A single-select, always-one-selected view/mode switcher — a bounded muted track with a raised active chip. The canonical segmented-control formula.
audience: public
registry: segmented
status: stable
since: 0.3.0
...
The track and chip are the system's one selected-chip recipe (the selected-chip recipe), shared with
