...
<Slider.Root>
  ...
  <Slider.Control>
    {/* the rail (muted by default; per-variant) */}
    <Slider.Track>
...
- **Track** — the rail spanning the full `min`→`max` range (`muted` on the `default`
...
<ApiTable path="../../packages/ui/registry/ui/slider.tsx" name="SliderProps" />
