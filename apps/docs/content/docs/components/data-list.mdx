...
Cells wrap by default (D18): a long value breaks inside its own column instead of forcing the whole
table to scroll. Two column shapes are unreadable when broken, so they opt back out automatically:

- `align: "end"` — the numeric convention.
- `mono: true` — renders values in the mono numeral face (`font-mono font-mono text-sm tabular-nums`) so
