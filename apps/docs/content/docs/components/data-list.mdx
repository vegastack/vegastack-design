...
[`Table`](/docs/components/table) using the shared
[`data-table-parts`](/docs/components/data-table-parts): the sort header, the selection cells, the
skeleton rows, the empty row and the column class rules it has in common with
[`DataGrid`](/docs/components/data-grid).
...
- `mono: true` — renders values in the mono numeral face (`font-mono text-code tabular-nums`) so
...
<ComponentChangelog name="data-list" />
