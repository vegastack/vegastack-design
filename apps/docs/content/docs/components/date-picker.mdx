...
The trigger label is formatted with native `Intl.DateTimeFormat`. Pass `formatOptions` for a custom shape and `locale` for a BCP-47 locale (here a long German date).
...
- Focus is always visible: the outline `Button` trigger brightens its border on `:focus-visible` (`focus-visible:border-ring/70`), and the day grid manages roving focus itself (react-day-picker focuses the active day on keyboard navigation). Today's cell additionally carries a neutral `ring` (`ring-ring/50`).
