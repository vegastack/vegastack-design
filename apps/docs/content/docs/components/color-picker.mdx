...
<InstallSteps name="color-picker" />
...
- Every control keeps the global 2px `:focus-visible` outline (`outline-ring`, 1px offset) that `base.css` paints — the trigger and the `ghost` swatch buttons alike. Nothing here sets `outline: none`, and no component adds a ring of its own.

| Key                                                | Action                                                          |
| -------------------------------------------------- | --------------------------------------------------------------- |
| <kbd>Enter</kbd> / <kbd>Space</kbd>                | Open the picker (on the trigger), or select the focused swatch. |
| <kbd>←</kbd> / <kbd>→</kbd>                        | Move one swatch within the grid.                                |
| <kbd>↑</kbd> / <kbd>↓</kbd>                        | Move one row — by `columns` swatches.                           |
| <kbd>Home</kbd> / <kbd>End</kbd>                   | Jump to the first or last swatch in the grid.                   |
| <kbd>Tab</kbd> / <kbd>Shift</kbd> + <kbd>Tab</kbd> | Leave the grid — roving tabindex makes it one tab stop.         |
| <kbd>Esc</kbd>                                     | Close the picker and return focus to the trigger.               |

<StatesTested name="color-picker" />
