...
- Focus is trapped inside the popup while open and restored to the trigger on close. For the default
  modal, VegaStack mirrors Base UI's live outside-tree markers to native `inert`; this keeps focus
  contained even when the browser transiently hands it to `<body>`, and preserves nested portal
  stacking through reference-counted ownership. Page scroll is locked. `modal={false}` and
  `modal="trap-focus"` keep their Base UI outside-interaction contracts and do not add native inert.
...
<ComponentChangelog name="dialog" />
