...
ring-width or ring-offset tokens; `--ring` (light `oklch(0.205 0 0)`, dark `oklch(0.922 0 0)`)
is the only knob. Upstream's `ring-3 ring-ring/50` glow is removed everywhere, and `design-lint`'s
`no-focus-ring-glow` rule keeps it from returning on the next upstream pull.

Text-entry controls are the deliberate exception: `Input`, `Textarea`, `Select`, and combobox inputs
tint their border on `:focus` (`focus:border-ring/70`) so caret and pointer focus remain visible
without adding an outer ring inside dense forms. Other controls use the shared `:focus-visible`
outline.
