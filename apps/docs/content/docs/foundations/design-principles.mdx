...
from **size, weight and color**, on Tailwind's stock scale.
...
- `shadow-md` is reserved for floating overlay surfaces.
...
<DoDont
  do="Separate surfaces with border-border and use shadow-md for overlays — nothing else."
...
`--muted-foreground` is the one deliberately sub-AA tone, reserved strictly for placeholders.
...
<DoDont
  ...
  dont="Strip focus outlines or rely on muted-foreground for anything but placeholders."
/>
