...
<Callout type="info">
Not every token is redefined in `.dark`. The saturated marks (`--primary` fill, the status + `--info` fills, the
`hover`/`active`/`foreground` states) are **theme-independent** — a `bg-destructive` button reads the
same on a light or dark page. Only the theme-aware tokens (surfaces, `--muted-foreground`, the `subtle`
/ `text` pairs, `--border`, `--ring`, `--overlay`) appear in both blocks. See [Colors](/docs/foundations/colors).
</Callout>
...
Change `--primary` and `--color-primary` follows automatically, so `bg-primary`, `bg-primary/10`, and
`ring-primary` all repaint at once.

The bridge also exposes the non-color scales as Tailwind theme keys:

- **Radius** — `--radius-sm` / `--radius-md` / `--radius-lg` (`--radius` aliases `--radius-lg` at `0.75rem`), plus a bridge-only `--radius-xl` = `calc(var(--radius-lg) + 0.25rem)`.
...
with 2px offset** — there are no separate ring-width or offset tokens. Override `--ring` and every
...
This is the **only** sanctioned `!important` in the system. For JS-driven motion (the optional `motion`
peer), wrap the tree in Framer Motion's `<MotionConfig reducedMotion="user">` so programmatic animations
honor the same preference. The specimen below collapses to its static state under reduced motion:
