...
| Animated icons | Per-icon micro-animations with an imperative `startAnimation()` handle (e.g. the check that draws its stroke) | All 440 `icon-*` mirror items | Hover icons in the [icon gallery](/docs/foundations/icons) |
...
Form controls (`Input`, `OTPInput`, `Checkbox`, `RadioGroup`) auto-shake once when they *transition*
...
`motion-pop-in`, `motion-enter-up`, `motion-shake` (from `@vegastack/design-tokens/utilities.css`) are the
...
```tsx
<div className="transition-colors ease-standard duration-fast" />
```
