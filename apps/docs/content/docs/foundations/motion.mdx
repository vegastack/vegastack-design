...
| `motion-enter-up`                    | Fades in while rising 4px — "slides up into place"                                                            | `FieldError` messages, `StaggeredTextReveal`                                            | Submit an empty required Field; the error slides up                              |
...
`Field` shakes once when it _transitions_ into invalid, taking every control it wraps with it —
...
one sanctioned _transition_ pair, for a docked control that stays mounted and flips `data-active`;
