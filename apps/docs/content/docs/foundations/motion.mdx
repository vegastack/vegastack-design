...
| `motion-enter-up`                    | Fades in while rising 4px — "slides up into place"                                                            | `FieldError` messages, chat `Message`/`Bubble` `animateIn` (opt-in), `StaggeredTextReveal`                 | Submit an empty required Field; the error slides up                              |
