...
- `CommandInput` "Search…", then the scope chips — All · Meetings · Tasks · Products · Families ·
  Accessories · Customers · Projects · Attributes · Pages — as a `ToggleGroup wrap deselectable={false}` inside `CommandFilters`. The chips are their own Tab stop after the
...
- `CommandFooter` with no children — its built-in `Kbd` hints ↵ Open · ⌘↵ / Ctrl ↵ New tab · Esc
  Close — hidden below `sm`. `CommandLoading` shows the animated searching icon.
