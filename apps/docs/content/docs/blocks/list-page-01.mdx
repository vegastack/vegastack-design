...
- `AppShellPage` › `PageHeader` h1 "Customers" with a "New customer" link, and under the title
  its `tabs` row: Mine | Team as default (pill) `Tabs`, with the Grid | List `ViewToggle` in
  `view`, pinned to the row's end.
- `FilterBar searchPlacement="start"` — the search first, then the `FilterBarFacet`s "Status: Any" and "Industry".
