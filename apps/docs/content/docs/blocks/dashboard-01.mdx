---
title: dashboard-01
description: A ready-to-pull AI-platform dashboard page — sidebar nav, stat cards, a usage chart, and a recent-activity list, built entirely from existing VegaStack components.
...
<ComponentPreview name="dashboard01Demo" file="../../packages/ui/registry/blocks/dashboard-01/page.tsx" />
...
2. **Hoist the shell into a layout, not a page.** This block's file manifest mirrors shadcn's
   ...
   between.
3. **Add the reduced-motion override.** This workspace's global `prefers-reduced-motion` reset
   (`packages/design-tokens/src/base.css`) targets `*, ::before, ::after` — it does **not** reach the
   browser's `::view-transition-*` pseudo-element tree (a separate top-layer tree the universal
   selector doesn't match). Until that token-layer gap is closed, add this to your app's global
   CSS yourself:
   ```css
...
<AutoTypeTable path="../../packages/ui/registry/blocks/dashboard-01/page.tsx" name="DashboardPageProps" />

<AutoTypeTable path="../../packages/ui/registry/blocks/dashboard-01/components/app-sidebar.tsx" name="AppSidebarProps" />
