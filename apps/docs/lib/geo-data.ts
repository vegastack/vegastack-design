// @vegastack geo-data@0.23.21 sha256-f3WXzgpyDR7sSx3Q/OwOF/ZNq/dvglABYbTjDX6e7Mc=

/**
 * geo-data — the geography datasets behind `CountrySelect` and `RegionSelect`, installed ONCE as
 * a `registry:lib` item (audit B8-02 / decision D27). Both selects declare it as a registry
 * dependency, so a consumer who installs both copies the ~60 KB of ISO data a single time — and a
 * third consumer (a shipping-rate table, a phone-code field) reads the same arrays instead of
 * embedding a fourth copy.
 *
 * Pure data + two lookups: no React, no imports, no client boundary.
 */
...
/**
 * The full ISO 3166-1 alpha-2 country set (code + English name + flag emoji). Extend it by
 * editing this array — no code change required.
 */
...
/**
 * States/provinces keyed by ISO 3166-1 alpha-2 country code, covering the countries with
 * administrative divisions worth picking from a list (US states, Canadian provinces, Australian
 * states/territories, Indian states/UTs, UK regions, plus the high-volume billing/shipping markets
 * across the Americas, Europe, Africa, the Middle East and Asia-Pacific). A country absent from
 * this map has no predefined subdivisions and falls back to a free-text input.
 */
...
/**
 * The states/provinces for a country code (case-insensitive). Returns an empty array when the
 * country has no predefined subdivisions — the caller falls back to free text.
 */
