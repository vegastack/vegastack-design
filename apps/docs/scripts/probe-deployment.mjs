...
import { writeJsonAtomic } from "../../../tooling/lib/deploy-candidate.mjs";
...
const usage = `Usage: node apps/docs/scripts/probe-deployment.mjs [--report <path>]

Verify the deployed public-site/private-registry boundary from an external runner. The command
requires DOCS_URL, CF_ACCESS_CLIENT_ID, CF_ACCESS_CLIENT_SECRET, SIGNER_REPOSITORY, and SIGNER_REF
for execution. --help never reads credentials or contacts production.

  --report <path>  atomically write a structured pass/fail report with every attempted probe
  --help           show this help without performing a probe`;
...
for (let index = 0; index < cliArgs.length; index++) {
  if (cliArgs[index] !== "--report") {
    console.error(
      `probe-deployment: unknown option ${cliArgs[index]}\n\n${usage}`,
    );
    process.exit(2);
  }
  reportPath = cliArgs[++index];
  if (!reportPath) {
    console.error(`probe-deployment: --report requires a path\n\n${usage}`);
    process.exit(2);
  }
}
if (reportPath) reportPath = resolve(reportPath);

let base = process.env.DOCS_URL ?? "unknown";
...
async function observe(name, operation) {
  try {
    ...
    observations.push({ name, state: "pass" });
    return value;
  } catch (error) {
    observations.push({ name, state: "fail", error: errorSummary(error) });
    throw error;
  }
}

function writeTerminalReport(terminalState, error) {
  if (!reportPath) return;
  writeJsonAtomic(reportPath, {
    schema: 1,
    kind: "vegastack-deployment-probe",
    state: terminalState,
    startedAt,
    ...
    passed: observations.filter((entry) => entry.state === "pass").length,
    failed: observations.filter((entry) => entry.state === "fail").length,
    observations,
...
async function expectText(pathname, expectedType, validate, options) {
  return observe(`public:${pathname}`, async () => {
...
async function expectProtected(pathname) {
  return observe(`anonymous-denial:${pathname}`, async () => {
    const response = await requestUntil(
      pathname,
      {},
      {
    ...
    console.log(
      `✓ ${pathname} rejects anonymous requests (${response.status})`,
    );
  });
}

async function expectPublicNoindex(pathname) {
  return observe(`anonymous-unlisted:${pathname}`, async () => {
    const response = await requestUntil(
      pathname,
      {},
      {
...
async function expectRetired(pathname) {
  return observe(`retired:${pathname}`, async () => {
    const response = await requestUntil(
      pathname,
      {},
      {
...
async function main() {
  base = productionOrigin(process.env.DOCS_URL);
  ...
  await expectText("/api/search", "application/json", (body, response) => {
    ...
    assert.doesNotMatch(
      body,
      /\/internal\//,
      "/api/search: internal URL leaked",
    );
    assert.ok(
      !body.includes(RETIRED_PUBLIC_ROUTE),
  ...
  await observe("service-token-registry-chain", async () => {
    ...
    const signatureDir = mkdtempSync(
      join(tmpdir(), "vegastack-live-signature-"),
    );
    try {
...
try {
  await main();
  writeTerminalReport("pass");
} catch (error) {
  try {
    writeTerminalReport("fail", error);
  } catch (reportError) {
    console.error(
      `probe-deployment: failed to write structured report: ${errorSummary(reportError).message}`,
    );
  }
  throw error;
}
