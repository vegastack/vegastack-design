...
const TOAST_CONTENT: Record<ToastIntent, { message: string; description: string }> = {
  default: { message: 'Event created', description: 'Friday, June 26 at 10:00' },
  success: { message: 'Project deployed', description: 'main@a1f7c2 is live' },
  error: { message: 'Could not save changes', description: 'Check your connection and try again' },
  warning: { message: 'Storage is almost full', description: '9.2 GB of 10 GB used' },
  info: { message: 'A new version is available', description: 'v2.4.0 — refresh to update' },
};
...
const sonnerPlaygroundConfig: PlaygroundConfig<SonnerPlaygroundKey> = {
  controls: [
    ...
    { type: 'switch', key: 'description', label: 'Description', defaultValue: false },
  ],
  render: (state): ReactNode => (
    <Button
      variant="outline"
      onClick={() => fireToast(state.intent as ToastIntent, Boolean(state.description))}
    >
      Show toast
    </Button>
  ),
