...
const itemPlaygroundConfig: PlaygroundConfig<ItemPlaygroundKey> = {
  controls: [
    ...
    { type: 'select', key: 'media', label: 'Media variant', options: MEDIA_OPTIONS, defaultValue: 'default' },
  ],
  render: (state): ReactNode => (
    // The outer div is preview-only chrome (constrains the row width); the generated JSX
    // mirrors the `Item` element itself.
    <div className="w-full max-w-md">
      <Item variant={state.variant as ItemProps['variant']} size={state.size as ItemProps['size']}>
  ...
  toCode: (state) => {
    ...
    if (state.variant !== 'default') rootProps.push(`variant="${state.variant}"`);
    ...
    const mediaChild = state.media === 'image' ? IMAGE_PLACEHOLDER_CODE : '<Mail />';
    return `<Item${root}>
  <ItemMedia${media}>
    ${mediaChild}
  </ItemMedia>
