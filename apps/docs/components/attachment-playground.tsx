...
type AttachmentPlaygroundKey = 'state' | 'size' | 'orientation' | 'media' | 'live';
...
const IMAGE_PLACEHOLDER = <div aria-hidden="true" className="size-full bg-gradient-to-br from-muted to-accent" />;

const IMAGE_PLACEHOLDER_CODE = '<div className="size-full bg-gradient-to-br from-muted to-accent" />';
...
const attachmentPlaygroundConfig: PlaygroundConfig<AttachmentPlaygroundKey> = {
  controls: [
    { type: 'select', key: 'state', label: 'State', options: STATE_OPTIONS, defaultValue: 'idle' },
    { type: 'select', key: 'size', label: 'Size', options: SIZE_OPTIONS, defaultValue: 'default' },
    { type: 'select', key: 'orientation', label: 'Orientation', options: ORIENTATION_OPTIONS, defaultValue: 'horizontal' },
    { type: 'select', key: 'media', label: 'Media variant', options: MEDIA_OPTIONS, defaultValue: 'icon' },
    { type: 'switch', key: 'live', label: 'Live description', defaultValue: false },
  ],
  ...
  toCode: (state) => {
    ...
    if (state.orientation !== 'horizontal') rootProps.push(`orientation="${state.orientation}"`);
    const root = rootProps.length > 0 ? ` ${rootProps.join(' ')}` : '';
    const media = state.media !== 'icon' ? ` variant="${state.media}"` : '';
    const mediaChild = state.media === 'image' ? IMAGE_PLACEHOLDER_CODE : '<FileText />';
