        ...
        <SheetContent>
          ...
          <div className="flex-1 overflow-y-auto px-4">
            <p className="text-muted-foreground">
              Your name and bio are visible to everyone in the workspace. Email changes require
              re-verification before they take effect.
            </p>
          </div>
          <SheetFooter>
        ...
        <Sheet key={side}>
          ...
          <SheetContent side={side}>
            <SheetHeader>
              ...
              <SheetDescription>
                This sheet slides in from the <code>{side}</code> edge of the screen.
              </SheetDescription>
            </SheetHeader>
            <div className="flex-1 overflow-y-auto px-4">
              <p className="text-muted-foreground">
                Side panels are flush to their pinned edge with no radius; a top or bottom panel
                keeps a small radius on its free edge.
              </p>
            </div>
            <SheetFooter>
              <SheetClose render={<Button variant="outline">Cancel</Button>} />
      ...
      <Sheet>
        ...
        <SheetContent showCloseButton={false}>
          <SheetHeader>
            ...
            <SheetDescription>
              With <code>showCloseButton={'{false}'}</code> there is no top-right <code>X</code> —
              dismiss the panel through an explicit footer action (or <kbd>Esc</kbd>).
            </SheetDescription>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto px-4">
            <p className="text-muted-foreground">
      ...
      <Sheet>
        ...
        <SheetContent className="max-w-md" closeLabel="Dismiss filters">
          <SheetHeader>
            ...
            <SheetDescription>
              A <code>className=&quot;max-w-md&quot;</code> override widens the panel past the
              default <code>max-w-sm</code> cap. The close button carries a custom{' '}
              <code>closeLabel</code> for screen readers.
            </SheetDescription>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto px-4">
            <p className="text-muted-foreground">
              Width overrides only apply to <code>left</code>/<code>right</code> sheets;{' '}
              <code>top</code>/<code>bottom</code> sheets are full-width and cap their height
              instead.
            </p>
          </div>
          <SheetFooter>
