...
import { DataList, type DataListColumn, type SortState } from '@/components/ui/data-list';
...
const STATUS: Record<Status, { label: string; color: 'success' | 'info' | 'destructive' }> = {
...
const people: Person[] = [
  { id: '1', name: 'Ada Lovelace', email: 'ada@vega.dev', role: 'Engineer', status: 'active', amount: 1280 },
  { id: '2', name: 'Bea Arthur', email: 'bea@vega.dev', role: 'Designer', status: 'invited', amount: 940 },
  { id: '3', name: 'Cole Train', email: 'cole@vega.dev', role: 'Manager', status: 'active', amount: 2150 },
  { id: '4', name: 'Dax Shepard', email: 'dax@vega.dev', role: 'Engineer', status: 'suspended', amount: 760 },
  { id: '5', name: 'Eve Polastri', email: 'eve@vega.dev', role: 'Analyst', status: 'active', amount: 1530 },
];
...
const columns: DataListColumn<Person>[] = [
  ...
  },
  {
    ...
    align: 'end',
    sortable: true,
    render: (p) => <span className="font-mono tabular-nums">${p.amount.toLocaleString()}</span>,
  },
];
...
export function dataList(): ReactNode {
  const [sort, setSort] = React.useState<SortState | null>({ key: 'name', direction: 'asc' });
  return (
...
export function dataListLoading(): ReactNode {
  return (
    <Wrapper className="block">
      <DataList columns={columns} data={[]} getRowId={(p) => p.id} loading loadingRows={4} />
    </Wrapper>
  );
}
