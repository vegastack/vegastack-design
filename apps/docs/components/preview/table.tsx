...
const STATUS: Record<InvoiceStatus, { label: string; color: 'success' | 'warning' | 'destructive' }> =
  {
...
const invoices: {
  ...
  { invoice: 'INV-003', status: 'overdue', method: 'Bank transfer', amount: '$350.00' },
  { invoice: 'INV-004', status: 'paid', method: 'Credit card', amount: '$90.00' },
];
        ...
        <TableBody>
          {invoices.map((row) => (
            <TableRow key={row.invoice}>
              <TableCell className="font-mono tabular-nums font-medium">{row.invoice}</TableCell>
              <TableCell>
                <Badge variant="subtle" intent={STATUS[row.status].color} dot size="sm">
      ...
      <Table>
        ...
        <TableBody>
          {invoices.slice(0, 3).map((row) => (
            <TableRow key={row.invoice} data-selected={row.invoice === 'INV-002' ? '' : undefined}>
              <TableCell className="font-mono tabular-nums font-medium">{row.invoice}</TableCell>
              <TableCell>
                <Badge variant="subtle" intent={STATUS[row.status].color} dot size="sm">
                  {STATUS[row.status].label}
        ...
        <TableFooter>
          <TableRow>
            ...
            <TableCell className="text-right font-mono tabular-nums">$750.00</TableCell>
          </TableRow>
        </TableFooter>
      </Table>
    </Wrapper>
