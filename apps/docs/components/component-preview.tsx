...
function getRegistryComponentName(file?: string): string | undefined {
  ...
  return existsSync(`../../packages/ui/registry/ui/${name}.tsx`) ? name : undefined;
}
...
export async function ComponentPreview({ name, file }: { name: string; file?: string }) {
  ...
  const toolbar = (
    <div className="flex items-center gap-2">
      <FrameWidthToggle />
      <FullscreenToggle />
      {componentName ? <CopyPromptButton componentName={componentName} /> : null}
    </div>
  );
