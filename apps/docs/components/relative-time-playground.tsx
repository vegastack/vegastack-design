...
const DATE_OPTIONS = [
  {
    ...
    label: "5 minutes ago",
    offsetMs: 5 * 60_000,
    code: "new Date(Date.now() - 5 * 60_000)",
  },
  {
    value: "26h",
    label: "26 hours ago",
    offsetMs: 26 * 3_600_000,
    code: "new Date(Date.now() - 26 * 3_600_000)",
  },
  {
    value: "40d",
    label: "40 days ago",
    offsetMs: 40 * 86_400_000,
    code: "new Date(Date.now() - 40 * 86_400_000)",
  },
  {
    value: "in3d",
    label: "In 3 days",
    offsetMs: -3 * 86_400_000,
    code: "new Date(Date.now() + 3 * 86_400_000)",
  },
] as const;
...
function dateOption(value: string | boolean) {
  return (
    DATE_OPTIONS.find((option) => option.value === value) ?? DATE_OPTIONS[0]
  );
}
...
const relativeTimePlaygroundConfig: PlaygroundConfig<RelativeTimePlaygroundKey> =
  {
    ...
    toCode: (state) => {
      ...
      if (state.unitStyle !== "long")
        props.push(`unitStyle="${state.unitStyle}"`);
