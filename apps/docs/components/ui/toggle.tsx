// @vegastack toggle@0.2.0 sha256-bzfpIzXWB8eYYQuB8Rxy/vt0xwIPUJM2z2kkXFOS4rQ=
...
export const toggleVariants = cva(
  ...
  "inline-flex shrink-0 items-center justify-center gap-1.5 rounded-md bg-transparent text-base font-medium whitespace-nowrap transition-all duration-fast ease-standard select-none hover:bg-muted hover:text-foreground data-pressed:bg-foreground/(--alpha-ink-tint) data-pressed:text-foreground hover:data-pressed:bg-foreground/(--alpha-ink-tint-strong) disabled:pointer-events-none disabled:opacity-(--opacity-dim) aria-invalid:border-destructive/(--alpha-tint-border) [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-(--icon-default)",
  {
    variants: {
      // Control heights on the shared 28 / 32 / 40 scale (h-(--size-sm) / h-(--size-md) / h-(--size-lg)).
      size: {
        sm: "h-(--size-sm) min-w-(--size-sm) gap-1 px-1.5 text-sm [&_svg:not([class*='size-'])]:size-(--icon-inline)",
...
export interface ToggleProps
  extends Omit<React.ComponentPropsWithRef<typeof BaseToggle>, 'value'>,
...
export function Toggle({ className, size = 'default', ...props }: ToggleProps) {
  ...
  const resolvedClassName: React.ComponentPropsWithRef<typeof BaseToggle>['className'] =
    typeof className === 'function'
      ? (state) => cn(variantClassName, className(state))
      : cn(variantClassName, className);

  return (
    <BaseToggle data-slot="toggle" data-size={size} className={resolvedClassName} {...props} />
  );
}
