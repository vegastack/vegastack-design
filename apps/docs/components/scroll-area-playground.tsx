...
const ROWS = Array.from({ length: 14 }, (_, i) => `Changeset ${String(i + 1).padStart(2, '0')}`);
...
function boxClassName(orientation: string | boolean): string {
  return orientation === 'horizontal' ? 'w-56 rounded-md border' : 'h-40 w-56 rounded-md border';
}
  ...
  if (orientation === 'horizontal') {
    return (
      <div className="flex w-max gap-2 p-3">
        {TAGS.map((tag) => (
          <span key={tag} className="rounded-md bg-muted px-2 py-1 text-sm whitespace-nowrap">
            {tag}
          </span>
        ))}
      </div>
...
const scrollAreaPlaygroundConfig: PlaygroundConfig<ScrollAreaPlaygroundKey> = {
  controls: [
    { type: 'select', key: 'orientation', label: 'Orientation', options: ORIENTATION_OPTIONS, defaultValue: 'vertical' },
  ],
