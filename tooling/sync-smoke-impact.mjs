...
import { SMOKE_MODEL } from "./lib/smoke-scope.mjs";
...
const vitest = await createVitest("test", {
  root: uiRoot,
  config: join(uiRoot, "vitest.smoke.config.ts"),
  run: true,
  watch: false,
...
const entries = {};
try {
  for (const [path, expected] of [...SMOKE_MODEL.impactByFile].sort(
...
console.log(
  `✓ smoke impact: ${Object.keys(entries).length} modeled source/test paths shadow-compared with Vitest related; ${disagreements} disagreement(s) widen`,
);
