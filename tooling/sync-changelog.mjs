...
if (firstEntry === -1) {
  console.error(
    '✗ sync-changelog: no "## [x.y.z]" entries found in CHANGELOG.md',
  );
  process.exit(1);
}
...
if (s === -1 || e === -1 || e < s) {
  console.error(
    `✗ sync-changelog: markers missing/misordered in ${PAGE}\n  expected "${START}" then "${END}"`,
  );
  process.exit(1);
}
const next =
...
if (check) {
  if (next !== page) {
    console.error(
      "✗ sync-changelog --check: docs changelog page is OUT OF SYNC with /CHANGELOG.md.",
    );
    console.error(
      "  Fix: node tooling/sync-changelog.mjs   (then commit the page)",
    );
    process.exit(1);
  }
  ...
  if (next !== page) {
    ...
    console.log(
      "✓ sync-changelog: docs changelog page regenerated from /CHANGELOG.md",
    );
  } else {
