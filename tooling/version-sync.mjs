...
console.log(`✓ version-sync: ${updated} item meta.version field(s) → ${version}`);
...
const problems = [];
let checked = 0;
for (const f of readdirSync(outDir).filter((n) => n.endsWith('.json') && !SKIP.has(n))) {
  const item = JSON.parse(readFileSync(join(outDir, f), 'utf8'));
  checked++;
  if (item.meta?.version !== version) {
    problems.push(`${f}: meta.version "${item.meta?.version}" ≠ package version "${version}"`);
  }
}
if (problems.length) {
...
console.log(`✓ version-sync: ${checked} built item(s) verified at v${version} (meta.version = package = headers)`);
