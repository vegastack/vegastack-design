...
export function operatorDocProblems(sources) {
  ...
  for (const [file, current] of runtimeExclusionSurfaces)
    if (
      ...
      !/exactly once[\s\S]{0,240}(?:excluded|reporter-excluded)[\s\S]{0,160}(?:listed(?:-and| and) passed|listed and\s+passed)/i.test(
        current,
      ) ||
      !/(?:absence|absent|missing)[\s\S]{0,160}(?:fail|block)/i.test(current)
    )
      problems.push(
        `${file}: [vitest-runtime-exclusion] must limit runtimeExclusions to five exact source-bound direct leaves, require each exactly once as excluded or listed-and-passed, and say missing/arbitrary leaves fail`,
      );
...
const semanticFixtures = [
  ...
  [
    "empty exclusions falsely prove capability recovery",
    "skills/internal/gates/SKILL.md",
    "runtimeExclusions permits exactly five source-bound leaves. Empty runtimeExclusions proves capability recovery even when approved leaves are absent. Any other arbitrary skip must fail.",
    /vitest-runtime-exclusion/,
  ],
];
