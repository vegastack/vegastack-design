...
| `DataList`                                             | Per-column `mobile` (`merge` · `visible` · `hidden`), `minWidth`, and `mergedRender` so a merged value keeps its context (`4 to review`). `DataGrid` adds editing, multi-key sort and a column picker; `Table` is static markup.                                                   |
